...
the_data <- inner_join(sep_old, sep_new, 
                       ...
                       suffix = c("_old", "_new")) %>% 
  mutate(differ = u_grad_new - u_grad_old)  
...
big_dec %>% select(title, name, department, differ) %>% 
gt() %>%
   tab_header(title = "Largest Decreases in Course Enrollment", 
              subtitle = "Harvard College Fall 2020")
